        ...
        if(fread(eocd_old, 1, eocd_size, file_sign) != eocd_size){
            ...
            if(memcmp(eocd, eocd_old, eocd_size) == 0){
                ...
                if((file_stage = fopen(RECOVERY_OTA_STAGE_FILE, "rb")) != NULL){
                    char buf[512] = {0};
                    int size=0;
                    size = fread(buf, 1, 512, file_stage);
                    if(strlen(buf) == 0){
...
LOGE("done < eocd_size1%d\n",eocd_size);
    size_t done = 0;
    while(done < eocd_size){
         LOGE("done < eocd_size%d\n",eocd_size);
...
static int handle_last_abort(int result){
    ...
    if(access(SYSRECOVERY,R_OK) == 0){
     ...
     LOGE("sysrecovery is not needed\n");

    LOG(ERROR) << "LastAbort: " << LastAbort;
    LOG(ERROR) << "result: " << result;
    LOG(ERROR) << "INSTALL_SUCCESS: " << INSTALL_SUCCESS;
    if(LastAbort && (access(SYSRECOVERY,R_OK) == 0) && (result != INSTALL_SUCCESS)){
       ...
       //set_bootloader_message(&bm);
        ...
        strlcpy(bm.command, "sysrecovery", sizeof(bm.command));
       // strlcpy(bm.recovery, "sysrecovery", sizeof(bm.recovery));
        std::string err;

        if (!write_bootloader_message(bm, &err)) {
...
static int really_install_package(const std::string& path, bool* wipe_cache, bool needs_mount,
  ...
  if(check_last_abort(check_path) != 0){
