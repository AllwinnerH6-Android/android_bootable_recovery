...
Value * MarkOtaStateFn(const char *name, State *state,  const std::vector<std::unique_ptr<Expr>>& argv){
     printf("set MarkOtaStateFn: \n");
    if(argv.size() != 1) {
        printf("set argv.size() != 1: \n");
            return ErrorAbort(state, kArgsParsingFailure, "%s() expects 1 arg, got %zu", name, argv.size());
    }

    std::vector<std::string> args;
    if(!ReadArgs(state, argv, &args)) {
         printf("set !ReadArgs(state, argv, &args: \n");
            return ErrorAbort(state, kArgsParsingFailure, "%s() Failed to parse the argument(s)", name);
    }
    const std::string& str = args[0];
    const char* constr=str.c_str();
    char* statestr=new char[100];
    strcpy(statestr,constr);
